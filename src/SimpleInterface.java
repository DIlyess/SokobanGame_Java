...
public class SimpleInterface extends JFrame implements KeyListener {
	...
	public SimpleInterface(String name, int width, int height) {
		...
		panel.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				lastClicked.add(new MyMouseEvent(e));
			}
		});
	...
	public void drawString(String s, double x, double y, int size, int[] color) {
		drawString(s, (int) (img.getWidth() * Math.max(0, Math.min(1, x))),
				(int) (img.getHeight() * Math.max(0, Math.min(1, y))), size, color);
		refresh();
	}
	 ...
	 * 
	 * @return the array [x,y,id] containing the clicked pixel (x,y) and the id of
	 *         the button
	 */
	public int[] popClick() {
